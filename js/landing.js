...
window.addEventListener('load', () => {
    ...
    (() => {
      ...
      tl.from(".hero-eyebrow", { opacity:0, y:20, duration:0.8 })
        ...
        .from(".hero-divider", { scaleX:0, transformOrigin:"left", duration:0.8 }, "-=0.6")
        .from(".hero-note", { opacity:0, y:20, duration:0.8 }, "-=0.5")
    ...
    // Handle window resize to adjust slide width dynamically
    window.addEventListener('resize', () => goToSlide(currentIndex));



    //=================================================================================
    // --- ABOUT SECTION: bulletproof mobile-safe animation + counter ---
    ...
    (() => {
        ...
        // 1️⃣ IntersectionObserver (mobile-safe)
        ...
        // 2️⃣ Immediate fallback (if already visible on load)
        window.addEventListener('load', () => {
    ...
    (() => {
        ...
        steps.forEach(step => {
            ...
            // Prepare for smooth GPU animations
            gsap.set(step, { willChange: 'transform, box-shadow' });

            const tl = gsap.timeline({ paused: true });

            // Hover animation: move up + smooth shadow fade-in
            tl.to(step, {
                y: -10,
    ...
    brandLogos.forEach(logo => {
      ...
      hoverTL.to(logo, {
        scale: 1.08,              // floating lift
        y: -8,
        rotationX: 5,
        boxShadow: "0px 0px 20px rgba(0, 0, 0, 0.7)", // glow
        duration: 0.2,
        ease: "power3.out"
      })
      .to(logo.querySelector('img'), {
        filter: "grayscale(0%)",   // full color reveal
        duration: 0.2,
        ease: "power2.out"
      }, 0); // start at same time as lift
    ...
    luxBtn.addEventListener("mouseenter", () => {
      ...
      gsap.to(luxBtn, {
        boxShadow:
          "0 35px 65px rgba(0,0,0,0.55), inset 0 1px 1px rgba(255,255,255,0.2)",
        duration: 0.5,
        ease: "power3.out"
      });
    });
    ...
    luxBtn.addEventListener("mouseleave", () => {
      ...
      gsap.to(luxBtn, {
        boxShadow:
          "0 25px 45px rgba(0,0,0,0.45), inset 0 1px 1px rgba(255,255,255,0.15)",
        duration: 0.5,
        ease: "power3.out"
      });
...
// ================= LUXURY CTA INTERACTIONS =================
(() => {
    'use strict';

    const buttons = document.querySelectorAll(
        '.final-cta .btn-primary, .final-cta .btn-whatsapp'
    );
    ...
    buttons.forEach(button => {
        ...
        hoverTl
            .to(button, {
                y: -10,
                scale: 1.045,
                boxShadow: '0 26px 35px rgba(0,0,0,0.5)',
                duration: 0.2,
                ease: 'power2.out'
            })
            // Gentle settle (luxury return-to-rest feel)
            .to(button, {
                y: -6,
                scale: 1.03,
                duration: 0.28,
        ...
        if (isTouch) {
            ...
            button.addEventListener('touchstart', (e) => {
                if (!revealed) {
                    ...
                    // Auto-reset if user doesn't tap again
                    setTimeout(() => {
                        hoverTl.reverse();
                        revealed = false;
                    }, 2200);
